# 👋 Hi, I'm Shubham Das

### 📊 Data Science | 🤖 Machine Learning | 🧠 Generative AI

I’m a data-driven professional with **4 years of professional experience**, currently focused on building my career in **Data Science, Machine Learning, and Generative AI**.

I enjoy transforming data into meaningful insights and building practical AI solutions using **Python, SQL, Machine Learning, NLP, LLMs, RAG, and Data Visualization**.

---

## 👨‍💻 About Me

* 🔭 Building projects in **Data Science & Generative AI**
* 🌱 Currently learning **Advanced Machine Learning, LLMs, RAG & AI Agents**
* 🧠 Interested in **Machine Learning, NLP, Generative AI & Data Analytics**
* 🐍 Strong focus on **Python & SQL**
* 📊 Experienced in **Data Analysis, Visualization & Business Analytics**
* 🤖 Building **LLM-powered and RAG-based applications**
* 🚀 Interested in developing **end-to-end AI solutions**
* 💡 Passionate about solving real-world problems using **data and AI**


## 🛠️ Tech Stack

### 🐍 Programming & Data

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge\&logo=python\&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge\&logo=postgresql\&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge\&logo=postgresql\&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge\&logo=pandas\&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge\&logo=numpy\&logoColor=white)

### 🤖 Machine Learning & Data Science

![Scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge\&logo=scikit-learn\&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-102230?style=for-the-badge\&logo=python\&logoColor=white)
![NLP](https://img.shields.io/badge/NLP-5C3EE8?style=for-the-badge)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge\&logo=matplotlib\&logoColor=white)
![Seaborn](https://img.shields.io/badge/Seaborn-4C72B0?style=for-the-badge)

### 🧠 Generative AI & RAG

![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge\&logo=langchain\&logoColor=white)
![LLMs](https://img.shields.io/badge/LLMs-412991?style=for-the-badge)
![RAG](https://img.shields.io/badge/RAG-0E7490?style=for-the-badge)
![FAISS](https://img.shields.io/badge/FAISS-0467DF?style=for-the-badge)
![Embeddings](https://img.shields.io/badge/Embeddings-6C63FF?style=for-the-badge)
![Generative AI](https://img.shields.io/badge/Generative%20AI-FF6F00?style=for-the-badge)

### 📊 Visualization & BI

![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge\&logo=power-bi\&logoColor=black)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=for-the-badge\&logo=plotly\&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge\&logo=streamlit\&logoColor=white)

### 🔧 Tools

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge\&logo=git\&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge\&logo=github\&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge\&logo=jupyter\&logoColor=white)
![Google Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?style=for-the-badge\&logo=googlecolab\&logoColor=black)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge\&logo=visual-studio-code\&logoColor=white)

---

## 🚀 Featured Projects

### 🔍 AI Market Research & Strategy Engine

An AI-powered application for transforming complex business questions into structured research insights using **AI agents, Generative AI, web research, and strategy-focused analysis**.

**Tech:** Python • Streamlit • Generative AI • Google Gemini • AI Agents • Web Research • Data Analysis

🔗 [View Repository](https://github.com/shubham444das-cpu/McKinsey-AI-Market-Research-Strategy-Engine.git)

---

### 🎬 Netflix Content Strategy Analysis

Data Science project focused on analyzing Netflix content using **EDA, feature engineering, visualization, and Machine Learning**.

**Tech:** Python • Pandas • NumPy • Scikit-learn • Matplotlib • Seaborn • Plotly

🔗 [View Repository](https://github.com/shubham444das-cpu/Netflix-Content-Strategy-Analysis)

---

### 🏠 Airbnb Operations Analytics Dashboard

Interactive **Power BI dashboard** analyzing Airbnb operations and performance across Chicago and New Orleans.

**Tech:** Power BI • DAX • Data Analysis • Data Visualization

🔗 [View Repository](https://github.com/shubham444das-cpu/Airbnb-Operations-Analytics-Dashboard)

---
### 🔬 Research Paper AI Assistant

An AI-powered application for analyzing research papers using **NLP, semantic search, RAG, and LLM-based techniques**.

**Tech:** Python • NLP • RAG • LLM • FAISS • LangChain • Streamlit

🔗 [View Repository](https://github.com/shubham444das-cpu/Research-Paper-AI-Assistant)

---


## 🧠 Areas of Interest

🤖 Generative AI
🧠 Machine Learning
🔎 Natural Language Processing
📚 Retrieval-Augmented Generation
🔗 Semantic Search
🧩 AI Agents
📊 Data Science
📈 Data Analytics
🗄️ SQL & Databases

---
## 💼 Professional Experience

### WPP

**Digital Marketing Executive**
*4 Years of Professional Experience*

* Worked with major brands including **Uber, Hotstar, SonyLIV, Disney, and Maruti**.

### AlmaBetter

**MS in Computer Science, Machine Learning and AI Engineering - Woolf University in collaboration with AlmaBetter**

Key areas:

* Python
* SQL
* Statistics
* Exploratory Data Analysis
* Machine Learning
* Natural Language Processing
* Generative AI
* Large Language Models
* Retrieval-Augmented Generation
* AI Agents

### 🎓 M.Sc. Chemistry

Developed a strong foundation in **analytical thinking, scientific methodology, quantitative reasoning, and problem-solving**.
## 🌱 Currently Learning

**Machine Learning → NLP → Generative AI → LLMs → RAG → AI Agents → Production AI**

---


## 🤝 Let's Connect

<p align="left">
  <a href="https://github.com/shubham444das-cpu">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>

---

### 💡 Turning Data Into Insights & Ideas Into Intelligent Solutions

⭐ Thanks for visiting my profile!
